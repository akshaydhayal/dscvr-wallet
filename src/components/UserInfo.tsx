...
export default function UserInfo({ user, onOpen }: UserInfoProps) {
  return (
    <a className="flex gap-4 group hover:text-indigo-400 hover:cursor-pointer" onClick={onOpen}>
      {user.avatar && <img src={user.avatar} alt="User Avatar" className="w-20 h-20 rounded-full" />}
